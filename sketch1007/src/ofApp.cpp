...
void ofApp::draw(){
  ...
  float petalsLength = ofMap(sin(ofGetElapsedTimef() * 0.5), -1, 1, 1,10);
  for(float t = 0; t < TWO_PI * 1; t += TWO_PI / (360 * petalsLength)) {
    int loop = t / TWO_PI + 1;
    ...
    ofSetRandomSeed(pow(t, 0.8)  );
    float offsetX = ofRandom(-kaos, kaos);
    ofSetRandomSeed(pow(2*t, 0.8));
